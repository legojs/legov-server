...
module.exports = function (config) {
  ...
  config.site[host][rewrite] = config.site[host][rewrite] || {port: 0, root: 'htdocs', view: 'views', asset: 'asset'};
  ...
  var service = http.createServer(function(req, res) {
    ...
    if (view && ext === '.html') {
      //默认查找路径为views
      pathname = [view].concat(pathname); 
    } else if (root) {
      //默认查找路径为htdocs
      pathname = [root].concat(pathname); 
    }

    pathname = pathname.join(path.sep);

    process.stderr.write("\n    Request for " +  pathname + " received.\n");
    fs.exists(pathname)
    .then(function (exist) {
      process.stderr.write(exist);
      process.exit(1);
      if (exist) {
    ...
    .then(function (content) {
      ...
      res.writeHead(status,{
        "Content-Type": mime[path.extname(pathname).replace('.', '')]
      });
